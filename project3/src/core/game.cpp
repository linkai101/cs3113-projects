...
void Game::init() {
  ...
  player = std::make_unique<Player>(
    getPositionFromTileCoordinates({ 2, 5 }, width, height),
    playerAnimator
  );
...
void Game::createBackgroundLayer(Texture2D texture) {
  float aspect = static_cast<float>(texture.width) / static_cast<float>(texture.height);
  float destHeight = static_cast<float>(height);
  float destWidth = destHeight * aspect;
  float x = (static_cast<float>(width) - destWidth) / 2.0f;
...
void Game::createTile(Vector2 tileCoordinates, int frameIndex, bool enablePhysics) {
  Vector2 position = getPositionFromTileCoordinates(tileCoordinates, width, height);
...
void Game::createGrass(Vector2 tileCoordinates, int frameIndex, bool hanging) {
  Vector2 position = getPositionFromTileCoordinates(tileCoordinates, width, height);
...
void Game::createWater(Vector2 tileCoordinates, bool surface) {
  Vector2 position = getPositionFromTileCoordinates(tileCoordinates, width, height);
...
void Game::resolveCollisions() {
  ...
  if (player->position.y - pb.getCollider(player->position).height > static_cast<float>(height) + 1000) {
    player->position = getPositionFromTileCoordinates({ 2, 5 }, width, height);
    pb.velocity = { 0, 0 };
    pb.acceleration = { 0, 0 };
...
Vector2 Game::getPositionFromTileCoordinates(Vector2 tileCoordinates, int width, int height) {
  return {
